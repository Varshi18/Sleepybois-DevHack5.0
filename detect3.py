...
#Turns out OpenAI API was paid so we had to drop the plan of smart assistant
...
# we were working on custom UI/UX design which could not be completed due to Time- constraints
        # #Process predictions
        ...
        #             # Show overlay video on detected object
        #             overlay_video = cv2.VideoCapture("D:\yolov5\hud.mp4")
        ...
        #             if roi.shape[0] > 0 and roi.shape[1] > 0:  # Ensure the region of interest is valid
        #                 overlay_frame_resized = cv2.resize(overlay_frame, (roi.shape[1], roi.shape[0]))
        #                 im0[xyxy[1]:xyxy[3], xyxy[0]:xyxy[2]] = cv2.addWeighted(roi, 1, overlay_frame_resized, 0.9, 0)
